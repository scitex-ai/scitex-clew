...
def verify_claims_dag(
    ...
    # Collect unique source files AND source sessions from claims.
    # source_session is the preferred anchor because claims-only DBs
    # (no runs in the file_hashes table) have no file-based lookup path.
    ...
    for c in claims:
        ...
        if c.source_session:
            source_sessions.add(c.source_session)

    # Resolve missing source_files via source_sessions (backward compat).
    if source_files or source_sessions:
        db = get_db()
        for sid in sorted(source_sessions):
            if sid:
                run = db.get_run(sid)
                if run and run.get("script_path") and run["script_path"] not in source_files:
                    source_files.append(run["script_path"])

    if not source_files and not source_sessions:
    ...
    # Build DAG from source_files (verify_dag will BFS back through session_parents).
    if source_files:
        return verify_dag(source_files)

    # No source_files at all — build DAG purely from session IDs.
    return _verify_claims_dag_from_sessions(sorted(source_sessions))


def _verify_claims_dag_from_sessions(session_ids: list[str]) -> DAGVerification:
    """Build a DAG purely from session IDs (no file-based lookup).

    Used when claims have source_session but no source_file or the
    file_hashes table is empty for those sessions (claims-only mode).
    """
    from collections import defaultdict, deque as _deque

    from ._chain import DAGVerification, VerificationStatus
    from ._chain._verify_ops import verify_run
    ...
    # BFS backward from leaf sessions to collect full DAG adjacency.
    adjacency: dict[str, list[str]] = {}
    all_ids: set[str] = set()
    queue: _deque[str] = _deque(session_ids)
    visited: set[str] = set()
    ...
    with db._connect() as conn:
        while queue:
            ...
            rows = conn.execute(
                "SELECT parent_session FROM session_parents WHERE session_id = ? ORDER BY recorded_at",
                (sid,),
            ).fetchall()
    ...
    # Topological sort (roots first) — mirrors _chain._dag._topological_sort
    def _topo_sort(adj: dict[str, list[str]]) -> list[str]:
        forward: dict[str, list[str]] = defaultdict(list)
        in_degree: dict[str, int] = {}

        for child, parents in adj.items():
            all_nodes = set(parents) | {child}
            for pn in all_nodes:
                if pn not in in_degree:
                    in_degree[pn] = 0
            for p in parents:
                forward[p].append(child)

        for node in set(adj.keys()) | {p for ps in adj.values() for p in ps}:
            if node not in in_degree:
                in_degree[node] = 0

        q = _deque(n for n in in_degree if in_degree[n] == 0)
        result = []
        while q:
            node = q.popleft()
            result.append(node)
        ...
        all_nodes = set(adj.keys()) | {p for ps in adj.values() for p in ps}
        if len(result) != len(all_nodes):
            raise ValueError("Cycle detected in DAG")
        return result

    topo_order = _topo_sort(adjacency)
