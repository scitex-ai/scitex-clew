...
def _raise_empty_view(output_path: Path) -> None:
    ...
    The pre-migration version of this message named the store FILE that had
    been found. There is no file: ``describe()`` is the store primitive's
    credential-free one-liner naming which store actually answered.
    """
    from .._db import get_db

    raise ValueError(
        f"scitex-clew store {get_db()._runs.target.describe()} answered but "
        f"the requested DAG view is empty — refusing to write {output_path}. "
...
def render_dag(
    ...
    ------
    ValueError
        If the output extension is unsupported, or if the requested view is
        empty (e.g. ``claims=True`` with zero claims, or filters matching
        nothing) — render_dag never returns without writing the requested
        file.

    Notes
    -----
    There is no ``db_path`` parameter. clew reads THE store this host uses
    (``host_store()``); rendering a store that lives outside the current
    tree — a capsule's own ``clew.db``, before the Postgres migration — is
    a capability this no longer has.
    """
