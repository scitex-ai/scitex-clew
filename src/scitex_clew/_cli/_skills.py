"""`scitex-clew skills` — list / get / install agent-facing skills.

Self-contained. No scitex-dev runtime dep — walks the package's own
`_skills/scitex-clew/` directory directly.
"""
...
def _skills_root() -> Path:
    """Resolve the bundled `_skills/scitex-clew/` directory."""
    import scitex_clew

    pkg_dir = Path(scitex_clew.__file__).parent
    return pkg_dir / "_skills" / PKG


def _list_skill_files(root: Path) -> list[Path]:
    """All `.md` files under `_skills/scitex-clew/` (recursive), excluding SKILL.md."""
    if not root.is_dir():
        return []
...
def skills_group(ctx) -> None:
    ...
    Examples:
      ...
      $ scitex-clew skills get 01_installation
      $ scitex-clew skills install                  # → ~/.scitex/dev/skills/scitex-clew/
      $ scitex-clew skills install --claude-symlink # also expose to ~/.claude/skills/scitex/
    """
...
def skills_get(name: str, as_json: bool) -> None:
    ...
    Example:
      $ scitex-clew skills get 01_installation
      $ scitex-clew skills get 02_quick-start --json
    """
    root = _skills_root()
...
@click.option(
    ...
    type=click.Path(),
    default=None,
    help="Destination dir (default: ~/.scitex/dev/skills/scitex-clew/).",
)
@click.option(
    "--no-link",
    "no_link",
...
def skills_install(
    ...
    \b
    Example:
      $ scitex-clew skills install
      $ scitex-clew skills install --claude-symlink
      $ scitex-clew skills install --no-link --dest /tmp/scitex-clew-skills
    """
